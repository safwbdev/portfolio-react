  ...
  <div>
    <Slider className="wow fadeIn" {...settings}>
      {props.array.map(function (item, key) {
        return (
          <div className="work row col xl12 l12 m12 s12" key={key}>
            ...
            <div className="col xl11 l11 m10 s12">
              <div className="work-detail">
                <span className="position">{item.work_position}</span>
                <span className="hide-on-small-only"> | </span>
                <br className="hide-on-med-and-up" />
                <span className="duration">
                  {" "}
                  <Moment format="MMMM YYYY">{item.work_start}</Moment> -{" "}
                  <Moment format="MMMM YYYY">{item.work_end}</Moment>
                </span>
              </div>
              <div className="work-detail">
                <span className="company">{item.work_name}</span>
                <span className="hide-on-small-only"> | </span>
                <br className="hide-on-med-and-up" />
                <span className="location">{item.work_location}</span>
              </div>
            ...
            <div className="hide-on-med-and-up">
              <span
                ...
                data-target={"descmodal" + key}
              >
                View Tasks & Responsibilities
              </span>
            </div>
          </div>
        );
    ...
    {props.array.map(function (item, key) {
      return (
        <Modal
          company={item.work_name}
          desc={getDesc(item.work_desc)}
          getId={"descmodal" + key}
          key={key}
        />
      );
